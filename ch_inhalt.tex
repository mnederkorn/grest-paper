...
\cite{Ehrenfeucht1979}\cite{ZWICK1996343}\cite{CONDON1992203}\cite{10.1007/978-3-540-85778-5_4}\cite{185392}There are practical applications (cite) which are able to be modeled by or reduced to various games. Considering the many ways there are to solve those games, the possibility to (repeatedly) reduce them to one another and various possibilities to implement any of those algorithms there are many potential means to solve problems on those games. We want to see how the different ways to solve problems, the reductions between those problems and the combination of the two play out and compare to one another in a real-world implementation.
\subsection{Task}
The games we concern ourselves here are \textit{Parity Games} (PG), \textit{Mean Payoff Games} (MPG), \textit{Energy Games} (EG), \textit{Discounted Payoff Games} (DPG) and (stopping) \textit{Simple Stochastic Games} (SSG). For any of those games there are multiple problems, such as ``what is the value of vertex $v$ under optimal strategies from both players?'' or ``what are the optimal strategies?''. For any of those problems, there are multiple ways to solve them for any specific game directly, some of which are abstractable to multiple of the games, such as Kleene Iteration and Strategy Iteration. Additionally we can reduce the games to another as following:
\begin{center}
	\begin{tikzpicture}
		\node (A) at (0,0) {$PG$};
	...
	\end{tikzpicture}
\end{center}
The reductions themselves are computationally relatively simple compared to the problems. Some of them however produce graphs that are bigger in some way ($PG\rightarrow MPG$, $DPG\rightarrow SSG$), which increases the complexity of subsequently applied algorithms, or reduce to multiple other game problems.
Another part we have control over is how exactly the algorithms are implemented. The biggest factor here being that, due to their nature, the edge-weights can be seen as an incidence matrix and can therefore make use of matrix operations as a form of parallelization for parts of the algorithms.
...
A Parity Game, $PG =(A,p)$, is played on an Arena $A$ with a priority function $p\colon V\rightarrow\{0, 1, \ldots, \left|V\right|\}$.\newline
...
\begin{wrapfigure}[9]{l}{5.5cm}
	\begin{tikzpicture}
		\node[style={regular polygon,regular polygon sides=4}, draw=black,label={$p(v_0)=5$}] (A) at (0,0) {\color{white}$v_0$};
		\node[style={regular polygon,regular polygon sides=4}, draw=black,label={$p(v_2)=4$}] (C) at (3.5,0) {\color{white}$v_2$};
		\node[style={regular polygon,regular polygon sides=4}, draw=black,label={below:$p(v_3)=2$}] (D) at (0,-1.75) {\color{white}$v_3$};
		\node[style={regular polygon,regular polygon sides=4}, draw=black,label={below:$p(v_5)=3$}] (F) at (3.5,-1.75) {\color{white}$v_5$};
		\node[shape=circle, draw=black,label={$p(v_1)=2$}] (B) at (1.75,0) {\color{white}$v_1$};
		\node[shape=circle, draw=black,label={below:$p(v_4)=0$}] (E) at (1.75,-1.75) {\color{white}$v_4$};
...
A Mean Payoff Game, $MPG = (A,\nu,d,w)$, is played on an Arena $A$, with threshold $\nu \in \mathbb{Z}$ and an edge-weight function $w\colon E\rightarrow\{-d, \ldots, -1, 0, 1, \ldots, d\}$, $d\in \mathbb{N}_0$.\newline
\textit{Max} wins play $\pi_{\sigma, \tau}(MPG, v_0)=\langle v_0, v_1, \ldots\rangle$ if
\begin{align*}
	\liminf\limits_{n\rightarrow \infty} \left(\dfrac{1}{n}\sum_{i=0}^{n-1}w((v_i,v_{i+1}))\right)\geq\nu.
\end{align*}
Alternatively we can also ask what, for a given play, the maximum threshold $\nu$ is with which \textit{Max} can still win the play. We call this the \textit{value} $v_{\sigma,\tau}(v)$ of a vertex $v$. The goal for \textit{Max} is to maximize the mean payoff to guarantee a win under the highest possible $\nu$ and for \textit{Min} to minimize the mean payoff as to guarantee a win under the lowest possible $\nu$. Optimal strategies are those that maximize/minimize the mean payoff for a given MPG regardless of the initial vertex.
...
An Energy Game, $EG = (A,c,d,w)$, is played on an Arena $A$, with initial credit $c\in\mathbb{N}_0$ and an edge-weight function $w\colon E\rightarrow\{-d, \ldots, -1, 0, 1, \ldots, d\}$, $d\in \mathbb{N}_0$.\newline
\textit{Charging} wins play $\pi_{\sigma, \tau}(EG, v_0)=\langle v_0, v_1, \ldots\rangle$ if
\begin{align*}
...
Alternatively we can also ask what, for a given play, the minimum initial credit $c$ is with which \textit{Charging} can maintain the winning condition. We call this the \textit{minimum initial credit} or \textit{value} $v_{\sigma,\tau}(v)$ of a vertex $v$. If no such value exists, e.g. when the vertex is of \textit{Depleting} and has a self-loop with negative edge-weight, we write $v_{\sigma,\tau}(v)=\infty$.
...
A Discounted Payoff Game, $DPG = (A,\nu,d,w,\lambda)$, is played on an Arena $A$, with threshold $\nu \in \mathbb{Z}$, an edge-weight function $w\colon E\rightarrow\{-d, \ldots, -1, 0, 1, \ldots, d\}$, $d\in \mathbb{N}_0$ and a discount factor $0<\lambda<1$.\newline
\textit{Max} wins play $\pi_{\sigma, \tau}(DPG, v_0)=\langle v_0, v_1, \ldots\rangle$ if
\begin{align*}
	(1-\lambda)\left(\sum_{i=0}^{\infty}\lambda^i\cdot w((v_i,v_{i+1}))\right)\geq\nu.
\end{align*}
Similar to MPGs we can also alternatively ask for a maximum winable threshold $v$ with which \textit{Max} can still win. Again we call this the \textit{value} $v_{\sigma,\tau}(v)$ of a vertex $v$. Optimal strategies are also defined analogously to MPGs.
...
We say that a SSG is \textit{stopping} if for every possible position in a play, so given two fixed strategies $\sigma, \tau$, there is a path to one of the sink vertices. All SSGs we will consider from here on out will be stopping-SSGs.
...
\end{gather*}\\
Playing $SSG$ with $\sigma=\{(v_0,v_1)\}$ and $\tau=\{(v_2,v_0)\}$, the respective optimal strategies, results in the values $v_{\sigma,\tau}=\{(v_0,.07),(v_1,.07),(v_2,.07),(v_3,.05),(\mathfrak{0},0),(\mathfrak{1},1)\}$
...
First we shall explain the general idea of Value Iteration and Strategy Iteration, as they can be generalized and used to solve multiple of the problems posed. Then we will explain the specific implementations of those as well as unique algorithms used to solve each of the games we are interested in.
...
\section{Solutions and Reductions in Practise}
