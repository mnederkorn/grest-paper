...
Furthermore $w$ can be rescaled into $w' = (w\cdot q)-p$ so that the $v(v)\neq\infty$ for solving as an EG corresponds to $v(v)\geq\frac{p}{q}$ for solving as a MPG.
This allows us to split the vertices into sets with arbitrary values $\geq\frac{p}{q}$ or $<\frac{p}{q}$.
...
\end{align*}
we can split the set of vertices a maximum of $\lceil log_2(|V|\cdot W)\rceil$ to be able to determine the exact rational value to each vertex if played as a MPG.

The problem this leaves us with is that this still requires solving the entire Game for the values of an EG multiple times.
