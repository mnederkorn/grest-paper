...
If $\Upsilon_{\sigma,\tau}(v_0)\coloneqq max(\#_\infty(\pi_{\sigma, \tau}(PG, v_0)))$, the value of vertex $v_0$ under $\sigma, \tau$ is even, then \textit{Even} wins and vice versa. Optimal strategies for \textit{Even}/\textit{Odd} maximize the amount of starting vertices resulting in an even/odd value respectively.
...
Playing $PG$ with $\sigma=\{(v_0,v_1),(v_2,v_5),(v_3,v_4),(v_5,v_2)\}$ and $\tau=\{(v_1,v_0),(v_4,v_1)\}$, the respective optimal strategies, results in play $\pi_{\sigma, \tau}(PG, v_0)=\langle v_0, v_1, v_0, ..\rangle$ for which $\Upsilon_{\sigma,\tau}(v_0)=5$ is odd and the play is therefore winning for \textit{Odd}.
...
\end{align*}
We call this the \textit{value} $\Upsilon_{\sigma,\tau}(v_0)$ of a vertex $v_0$. Optimal strategies are those that maximize/minimize the value of the vertices.
...
$\Upsilon_{\sigma,\tau}\colon v_0\mapsto 2, v_1\mapsto 2, v_2\mapsto -4, v_3\mapsto 2, v_4\mapsto 1$\\\\\\
...
\end{align*}
We call the smallest inital credit that can sustain the winning condition for a given play the \textit{minimum initial credit} or \textit{value} $\Upsilon_{\sigma,\tau}(v)$ of a vertex $v$. If no such value exists, e.g. when the vertex is of \textit{Depleting} and has a reflexive edge with negative weight, we write $\Upsilon_{\sigma,\tau}(v)=\infty$. For any given position $\langle v_0, v_1, \ldots, v_k\rangle$ in a play we call $\left(\sum_{i=0}^{k-1}w((v_i,v_{i+1}))\right) + c$ the \textit{energy level} at that position. Keep in mind that \textit{Charging} doesn't necessarily aim to maximise their energy level at any specific position but rather to maintain a non-negative energy level at \textit{every} position.

The goal for \textit{Charging} is to avoid getting trapped in cycles with net negative edge-weight, as these can deplete any initial credit given, as well as to minimize the initial credit necessary to be compliant with the winning condition in all other cycles.
...
Playing $EG$ with $\sigma=\{(v_0,v_0)\}$ and $\tau=\{(v_1,v_0),(v_2,v_2),(v_3,v_0),(v_4,v_3)\}$, respective optimal strategies, results in the values $\Upsilon_{\sigma,\tau}\colon v_0\mapsto 0,v_1\mapsto 0,v_2\mapsto \infty,v_3\mapsto 0,v_4\mapsto 1$.
...
\end{align*}
Again we call this the \textit{value} $\Upsilon_{\sigma,\tau}(v)$ of a vertex $v$, we will round them from here on out. Optimal strategies are those that maximize/minimize the discounted payoff for a given MPG regardless of the initial vertex.
...
Playing $DPG$ with $\sigma=\{(v_0,v_0)\}$ and $\tau=\{(v_1,v_0),(v_2,v_2),(v_3,v_0),(v_4,v_4)\}$, the respective optimal strategies, results in the values $\Upsilon_{\sigma,\tau}\colon v_0\mapsto 2,v_1\mapsto 2.1,v_2\mapsto -4,v_3\mapsto 1.95,v_4\mapsto 1$.
...
Since the result of a play $\pi_{\sigma, \tau}(SSG, v_0)$ can be probablistic, it is assigned a probabilty to reach the $\mathfrak{1}$ sink rather than a fixed value.
...
The value $\Upsilon_{\sigma,\tau}(v)$ of a vertex $v$ is the probability to reach the $\mathfrak{1}$ sink vertex.
...
\end{gather*}\\
Playing $SSG$ with $\sigma=\{(v_0,v_1)\}$ and $\tau=\{(v_2,v_0)\}$, the respective optimal strategies, results in the values $\Upsilon_{\sigma,\tau}\colon v_0\mapsto .07,v_1\mapsto .07,v_2\mapsto .07,v_3\mapsto .05,\mathfrak{0}\mapsto 0,\mathfrak{1}\mapsto 1$.
...
\begin{align}
	\Upsilon(v_0)&=~(1-\lambda)\sum_{i=0}^{\infty}\left(\lambda^i\cdot w((v_i,v_{i+1}))\right)\nonumber\\
...
We can take \hyperref[l_eins]{(1)} and add W to all the weights. This makes is so that all weights are effectively $\geq0$, the value of the game is increased by $W$ and we can simplify to a lower bound of $\Upsilon(v_0)$:
\begin{align*}
	&\Upsilon(v_0)+W\\
...
\begin{align*}
	\Upsilon(v_0)\geq~&\frac{1}{l}\sum_{j=0}^{l-1}w((v_{j+k},v_{j+1+k}))-n(1-\lambda)\left(W+\frac{1}{l}\sum_{j=0}^{l-1}w((v_{j+k},v_{j+1+k}))\right)\\
...
Unlike all other reductions in this paper, this particular reduction doesn't reduce a MPG to a singular EG which is equivalent to the original MPG in some sense.
Rather it relies on repeatedly solving a rescaled EG, derived from the original MPG. Each of those solved EGs restricts the possible range of values any given vertex can assume and we repeat the process until only a single possible value remains for each vertex.

Any vertex in $A, w$ from which \textit{Charging}/\textit{Max} can force to reach a terminal cycle with net-weight $\geq0$ is necessarily exactly equal to a vertex with value $\Upsilon(v)\geq0$ if played as a MPG and a vertex with value $\Upsilon(v)\neq\infty$ if played as a EG.
We can use this to solve for the values of $A, w$ as an EG and subsequently make inferences about which vertices have a value $\geq0$ or $<0$ if played as a MPG.
Furthermore $w$ can be rescaled into $w' = (w\cdot q)-p$ such that the $\Upsilon(v)\neq\infty$ of the game solving as an EG corresponds to $\geq\frac{p}{q}, <\frac{p}{q}$ of solving as a MPG.
This allows us to determine the value of vertices as $\geq\frac{p}{q}$ or $<\frac{p}{q}$ for any arbitrary $\frac{p}{q}$.
Given that we know, as laid out in \hyperref[mpg_frac]{3.1.4}, that the value of the vertices of a MPG are all in the set
\begin{align*}
	& S=\left\{\frac{p}{q}\in\mathbb{Q}\mid  -q\cdot W\leq p\leq q\cdot W\land 1\leq q\leq |V|,~p\in \mathbb{Z},~q\in\mathbb{N}\right\}
\end{align*}
we can repeatedly split the set of possible values a vertex can have, by solving as an EG using an adequate $\frac{p}{q}$ up to a maximum of $\lceil log_2(|V|\cdot W)\rceil$ times, to be able to determine the exact value of a vertex. The $\frac{p}{q}$ chosen is the one that splits the current $S$ in half, i.e. the mean of $S$ or if that is not an element of $S$ itself the next highest element of $S$.

The problem this leaves us with is that this still requires solving EGs on the whole game graph $\lceil log_2(|V|\cdot W)\rceil$ times to get the value of a single vertex of the MPG.

To alleviate this problem we make use of an observation.\\
While every single vertex \textit{can} have a distinct value this is rarely the case. In MPGs all vertices in the same terminal cycle and those vertices leading to that cycle have, per definition, the same value. Furthermore, as shown in \cite{Brim2011}, if subsets of vertices are shown to have a different value they may be considered as entirely disconnected games for the purpose of solving for their value and optimal strategy without affecting such strategies or values. This is particularly of interest because any $\geq\frac{p}{q}, <\frac{p}{q}$ split that results in two {non-empty subsets of vertices means we can continue our search for values and strategies on two smaller subgraphs with differently restricted possible values $S$. The \textit{smaller} being significant because it means the algorithm that solves EGs will, in sum, run faster on the two subgraphs than it would on the whole game graph once.\newpage

\textbf{Example 8:} We reduce solving $MPG$ to solving via EGs.

We have the given MPG. $|V|$ is 6 and $W$ is 3. The initial set of possible values of the vertices is $S=\{\frac{-18}{6}, \frac{-17}{6}, \ldots \frac{-17}{6}, \frac{18}{6}\}$. If we solve for the values of $A, w$ as an EG we get $v_0, v_1, v_3, v_4 = \infty$ and  $v_2, v_5 \neq \infty$.
\begin{center}
	\begin{tikzpicture}
		\begin{scope}
				\node[style=circle, draw=black] (A) at (0,0) {$v_0$};
				\node[style=circle, draw=black] (B) at (1.75,1.75) {$v_1$};
				\node[style={regular polygon,regular polygon sides=4}, draw=black] (C) at (3.5,0) {$v_2$};
				\node[style={regular polygon,regular polygon sides=4}, draw=black] (D) at (0,-1.75) {$v_3$};
				\node[style=circle, draw=black] (E) at (1.75,-1.75) {$v_4$};
				\node[shape=circle, draw=black] (F) at (3.5,-1.75) {$v_5$};
				\path [->] (A) edge node[above]{-2} (C);
				\path [->] (A) edge[out=285, in=75] node[right]{1} (D);
				\path [->] (B) edge node[right]{2} (A);
				\path [->] (B) edge[out=330, in=120] node[above]{1} (C);
				\path [->] (C) edge[out=150, in=300] node[left]{0} (B);
				\path [->] (C) edge[out=285, in=75] node[right]{1} (F);
				\path [->] (D) edge[out=105, in=255] node[left]{-2} (A);
				\path [->] (D) edge node[above]{1} (E);
				\path [->] (E) edge[out=345, in=285, looseness=4] node[right]{-2} (E);
				\path [->] (E) edge node[above]{-3} (F);
				\path [->] (F) edge[out=105, in=255] node[left]{2} (C);
		\end{scope}
	...
	\end{tikzpicture}
\end{center}
From that we learn that in $MPG$ $v_0, v_1, v_3, v_4$ all have a value $<0$ and $v_2, v_5$ have a value $\geq0$. We also know that we can now solve for the values and optimal strategies of the two sets of vertices seperately without affecting the outcome.

We continue with only $v_0, v_1, v_3, v_4$ for which we can reduce the possible values to\\
$S=\{\frac{-18}{6}, \frac{-17}{6}, \ldots \frac{-1}{6}\}$ given that we excluded everything $\geq 0$ in the previous iteration. We set $\frac{p}{q}=\frac{-9}{6}$ and $w' = (w\cdot 6)-(-9)$, and solve as an EG again which results in $v_4 = \infty$ and $v_0, v_1, v_3 \neq \infty$.
\begin{center}
	\begin{tikzpicture}
		\begin{scope}
				\node[style=circle, draw=black] (A) at (0,0) {$v_0$};
				...
				\node[style={regular polygon,regular polygon sides=4}, draw=black] (D) at (0,-1.75) {$v_3$};
				\node[style=circle, draw=black] (E) at (1.75,-1.75) {$v_4$};
				\path [->] (A) edge[out=285, in=75] node[right]{15} (D);
				\path [->] (B) edge node[right]{21} (A);
				\path [->] (D) edge[out=105, in=255] node[left]{-3} (A);
				\path [->] (D) edge node[above]{15} (E);
				\path [->] (E) edge[out=345, in=285, looseness=4] node[right]{-3} (E);
		\end{scope}
		\begin{scope}[xshift=6cm]
				\node[style=circle, draw=black] (A) at (0,0) {$v_0$};
				\node[style=circle, draw=black] (B) at (1.75,1.75) {$v_1$};
	...
	\end{tikzpicture}
\end{center}
$v_0, v_1, v_3$ must have value $\geq \frac{-9}{6}$ (and $<0$ given the previous step) and $v_4$ must have value $<\frac{-9}{6}$.

We repeat for $v_0, v_1, v_3$ with $S=\{\frac{-9}{6}, \frac{-8}{6}, \ldots \frac{-1}{6}\}$ and $\frac{p}{q}=\frac{-5}{6}$ then $S=\{\frac{-5}{6}, \frac{-4}{6}, \ldots \frac{-1}{6}\}$ and $\frac{p}{q}=\frac{-3}{6}$ then finally $S=\{\frac{-3}{6}, \frac{-2}{6}, \frac{-1}{6}\}$ and $\frac{p}{q}=\frac{-2}{6}$ which only leaves $S=\{\frac{-3}{6}\}$ which gives of the value of $v_0, v_1, v_3$ as $-0.5$ under the original MPG.\\
Similarly we chase the branches of the binary tree for $v_2, v_5$ from the first step and $v_4$ from the second step until we ultemately find their value to be $1.5$ and $-2$ respectively.\newpage
...
\begin{center}
	...
	\end{tikzpicture}
\end{center}
Random vertices then have value $\Upsilon(u')=\lambda\cdot \Upsilon(v)+(1-\lambda)\cdot w'(e)$ which exactly equal to the value of $u$ in the DPG with rescaled edge-weights given that $e$ is chosen as the outgoing edge. Since all outgoing edges will be replaced by this construct, the value of $u$ is equal in the rescaled DPG and the SSG regardless of the strategies played.
