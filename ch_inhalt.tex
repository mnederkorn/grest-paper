...
Any vertex in $A, w$ from which \textit{Charging}/\textit{Max} can force to reach a terminal cycle with net-weight $\geq0$ is necessarily exactly equal to a vertex with value $v(v)\geq0$ if played as a MPG and a vertex with value $v(v)\neq\infty$ if played as a EG.
