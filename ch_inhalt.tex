...
The \textit{special value} that signals that the edge the matrix entry represents doesn't exist will generally, depending on context, be the minimum/maximum value the datatype can represent or NAN. If we represent matrices here, we will use $\times$ to represent the special values. For operations on those matrices we will pretend here that those values are effectively invisible.\\
...
\end{bmatrix}
\]
And we call such ownership matrices $O$. $0$ or $False$ denotes ownership to player 0 or similar, $1$ or $True$ to player 1 and for SSGs we also have $2$ for \textit{Random} nodes.\\
For dissecting matrices we will use the following notation: Given a matrix A, for which we only want to keep the edges for which the source and target vertex are in $su = [0,1,4]$, we write $A[su,su]$.
\begin{figure}[H]
	\centering
...
The code as represented here is a simplifications of the actual code. Mostly in regards to the handling of the matrix entries that correspond to non-existing edges or particularities of python or numpy.
...
\subsubsection{SSGs}
For SSGs we have an additional matrix, \textit{AVG}, that gives us the transition probability from any \textit{Average} vertex to it's successor vertices.
...
\textbf{L.21-24:} Unlike the degrees of freedom the opponent has in choosing a successor vertex which results in a half-space in the Linear Program, \textit{Average} just has an equality relationship in dependance of it's successors and transition probabilities.\\
...
\textbf{L.6:} For every iteration we generate the entire matrix of possibilites of outgoing edges for every vertex and the accompanying value.\\
\textbf{L.7-9:} We continue with the maximum/minimum value for the respective player's vertices.\\
\textbf{L.12-14:} Given the values of the game we can also read out optimal strategies based off of the successor vertices needed to reach the already calculated values. For \textit{Average} we simply leave these as -1 as to not disturb the indexing.
\newpage
