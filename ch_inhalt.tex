...
For a given $PG = (A,p)$ we  transform into a $MPG = (A,w)$, with the underlying arena $A$ being the same and the weights $w$ of $MPG$ based directly on the priority function $p$ of $PG$. Optimal strategies in $PG$ translate one to one to $MPG$ and the vertex $v$ being winning for \textit{Even} in $PG$ translates to $v$ having a value $>=0$ in $MPG$. Recall that for both $PGs$ and $MPGs$ winning strategies are memoryless.

Consider any play $\pi$ in $A$ with $n=|V|$ in accordance to strategy $\sigma$ of \textit{Even/Max}. Given that $A$ is finite and $\pi$ can continue infinitely, any such play, regardless of starting vertex $v_0$ will necessarily have a repeat vertex after a path of length $k\leq n$ and will then, by virtue of the strategies being memoryless, indefinitely continue in a cycle of length $l\leq n-k$.
...
\begin{align}\label{ref1}
	w((u,v)) = (-1)^{p(u)}\cdot (sum\langle z\in \{|w((x,y))|\mid (x,y)\in E\}\mid x\in V_u\footnotemark\rangle +1)
\end{align}
\footnotetext{$V_u = \{ v\in V\mid p(x)<p(u)\land (p(x)\neq p(u)~\mathrm{mod}~2)\}$ and therefore $V_u = \emptyset$ for $u = min\{p(v)\mid v\in V\}$}
This way the vertex with highest priority has an outgoing edge which, per definition, dominates the value of the MPG since it has a higher absolute weight than all vertices of lower priority of the opponent, regardless of their counterstrategy $\tau$. Thus any play on PG whose value is \textit{Even/Odd} will have a value of $\geq 1/\leq -1$ when translated to an MPG in such a way and any winning strategy in \textit{PG} will accordingly be a winning strategy in \textit{MPG}.\\
Conversely, any MPG that will be reduced-to in such a way will have an edge in it's terminal cycle whose absolute weight will be greater than the sum of all the edges of the opponent in said cycle. The vertex said greatest edge emanates from being exactly equivalent to the vertex with highest priority in the original PG due to \eqref{ref1}. Thus any play on such MPG whose value, or equivalently whose greatest edge, is $\geq 1/\leq -1$ will have \textit{Even/Odd} value in the original MPG and any winning strategy in MPG will be a winning strategy in PG.
\begin{wrapfigure}[7]{l}{4.5cm}
	\begin{tikzpicture}
	...
	\begin{tikzpicture}
		...
		\path [->] (B) edge[out=25, in=155] node[above]{-12} (C);
		\path [->] (C) edge[out=195, in=345] node[above]{5} (B);
		\path [->] (B) edge node[left]{-12} (E);
		\path [->] (F) edge node[right]{-2} (C);
		\path [->] (E) edge[out=60, in=210] node[left]{1} (C);
		\path [->] (C) edge[out=240, in=30] node[right]{5} (E);
		\path [->] (D) edge node[left]{5} (A);
		\path [->] (D) edge node[below]{5} (E);
		\path [->] (E) edge[out=345, in=285, looseness=4] node[right]{1} (E);
	\end{tikzpicture}
	\caption{MPG}
\end{wrapfigure}
...
\begin{tabular}{c|c|c|c|c}
	...
	$v_4$ & $0$ & $\emptyset$ & $\emptyset$ & 1 \\\hline
	$v_0$ & $1$ & $\{v_4\}$ & $\langle 1\rangle$ & -2 \\\hline
	$v_5$ & $3$ & $\{v_4\}$ & $\langle 1\rangle$ & -2 \\\hline
	$v_2, v_3$ & $4$ & $\{v_0, v_5\}$ & $\langle 2, 2\rangle$ & 5 \\\hline
	$v_1$ & $5$ & $\{v_2, v_3, v_4\}$ & $\langle 5, 5, 1\rangle$ & -12 \\\hline
\end{tabular}\\\\\\
\footnotetext{$\langle z\in \{|w((x,y))|\mid (x,y)\in E\}\mid x\in V_u\rangle$}
Here we can limit $d$ to $12$ instead of $|(-6)^5|=7776$\newpage
...
\begin{align*}
	...
	&\frac{(1-\lambda)\lambda^k}{1-\lambda^l}\sum_{j=0}^{l-1}\lambda^{j}\cdot w((v_{j+k},v_{j+1+k}))+(1-\lambda)\left(\sum_{i=0}^{k-1}\lambda^i\cdot w((v_i,v_{i+1}))\right)\\
\end{align*}
Now if we let $\lambda\to 1^-$ we get:
\begin{align*}
	&\lim_{\lambda\to 1^-}\left(\frac{(1-\lambda)\lambda^k}{1-\lambda^l}\sum_{j=0}^{l-1}\lambda^{j}\cdot w((v_{j+k},v_{j+1+k}))+(1-\lambda)\left(\sum_{i=0}^{k-1}\lambda^i\cdot w((v_i,v_{i+1}))\right)\right)=\\
